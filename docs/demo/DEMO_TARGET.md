...
This document describes a reproducible way to evaluate Kokuli against a deliberately vulnerable AI-style HTTP service without coupling Kokuli to another repository.

## Goal

Provide a lightweight path for reviewers, principals, founders, or security engineers to see Kokuli produce:
...
Deliberately leave some routes open and require auth on others so Kokuli can fingerprint mixed posture:
...
```bash
kokuli target add demo-vuln http://127.0.0.1:8088 --chat /chat --format input
kokuli target set demo-vuln
```

3. Run a focused suite:

```bash
kokuli suite security
kokuli suite recon
kokuli suite child-safety
```
...
Use the demo target to walk a reviewer through Kokuli’s intended workflow:
...
If the target fingerprint changes significantly between runs, Kokuli will warn that the comparison may not be directly comparable. That is expected and should be explained during a demo.
...
This guide is docs-only in this pass. That keeps Kokuli standalone and avoids turning the repository into a bundled target simulator. If public evaluation becomes a priority, the next step would be adding a tiny optional demo server under `examples/` with no runtime coupling to Kokuli itself.
